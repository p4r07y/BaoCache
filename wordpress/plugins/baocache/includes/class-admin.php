...
final class BaoCache_Admin {
	...
	public function render(): void {
		...
		<div class="wrap baocache">
			...
			<section class="baocache-dashboard-shell">
				<?php if ( is_multisite() ) : ?><section class="baocache-panel baocache-callout is-warning" data-baocache-pane="dashboard"><strong><?php esc_html_e( 'Multisite chưa được hỗ trợ production', 'baocache' ); ?></strong><span><?php esc_html_e( 'BaoCache đang chạy trong network. Stable RC yêu cầu test network activation, per-site isolation, invalidation, uninstall và rollback trước khi dùng production.', 'baocache' ); ?></span></section><?php endif; ?>
				...
				<?php $this->onboarding_panel( $status ); ?>
				...
				<?php $advisor_recommendations = BaoCache_Optimization_Advisor::recommendations(); ?>
				<section class="baocache-panel baocache-recommendations baocache-optimization-advisor"><div class="baocache-panel__heading"><div><h2><?php esc_html_e( 'Optimization Advisor', 'baocache' ); ?></h2><p><?php esc_html_e( 'Xếp hạng bằng evidence thật. Advisor không tự áp dụng và không vượt qua QA, fingerprint hoặc rollback của từng module.', 'baocache' ); ?></p></div><span class="baocache-badge is-neutral"><?php esc_html_e( 'beta83 · Evidence only', 'baocache' ); ?></span></div><?php if ( empty( $advisor_recommendations ) ) : ?><p class="baocache-analysis-note"><?php esc_html_e( 'Chưa có evidence liên module để đưa khuyến nghị.', 'baocache' ); ?></p><?php else : ?><div class="baocache-recommendations__list"><?php foreach ( $advisor_recommendations as $recommendation ) : ?><div><span class="baocache-priority is-<?php echo esc_attr( (string) $recommendation['risk'] ); ?>"><?php echo esc_html( strtoupper( (string) $recommendation['risk'] ) ); ?></span><strong><?php echo esc_html( (string) $recommendation['title'] ); ?></strong><span><?php echo esc_html( (string) $recommendation['mechanism'] . ' ' . (string) $recommendation['evidence'] . ' Phạm vi: ' . (string) $recommendation['scope'] ); ?></span><button type="button" class="button-link" data-baocache-go="<?php echo esc_attr( (string) $recommendation['tab'] ); ?>"><?php echo esc_html( (string) $recommendation['action'] ); ?></button></div><?php endforeach; ?></div><?php endif; ?></section>
			...
			<section class="baocache-panel baocache-cloudflare-audit">
				...
				<?php endif; ?>
				<?php if ( ! empty( $cloudflare_configuration['purge_enabled'] ) ) : ?><div class="baocache-cloudflare-audit__action"><div><strong><?php esc_html_e( 'Exact URL purge', 'baocache' ); ?></strong><small><?php esc_html_e( 'Chỉ URL công khai cùng domain, từng URL một. Không có purge all, host, prefix hoặc tag.', 'baocache' ); ?></small></div><input type="url" data-baocache-cloudflare-purge-url value="<?php echo esc_attr( home_url( '/' ) ); ?>" aria-label="<?php esc_attr_e( 'Cloudflare URL cần purge', 'baocache' ); ?>"><button type="button" class="button button-secondary" data-baocache-cloudflare-purge><?php esc_html_e( 'Purge URL', 'baocache' ); ?></button></div><?php elseif ( $cloudflare_configuration['configured'] ) : ?><div class="baocache-callout is-neutral"><strong><?php esc_html_e( 'Purge đang khóa', 'baocache' ); ?></strong><span><?php esc_html_e( 'Dùng purge token riêng có quyền Cache Purge, đặt BAOCACHE_CLOUDFLARE_PURGE_ENABLED=true và BAOCACHE_CLOUDFLARE_PURGE_API_TOKEN trong Coolify.', 'baocache' ); ?></span><small><?php echo esc_html( implode( ' · ', (array) ( $cloudflare_configuration['purge_missing'] ?? array() ) ) ); ?></small></div><?php endif; ?>
	...
	private function onboarding_panel( array $status ): void {
		$inventory = get_transient( 'baocache_asset_inventory' );
		$inventory_ready = is_array( $inventory ) && ! empty( $inventory['captured_at'] );
		$steps = array(
			array( 'state' => (string) ( $status[0]['state'] ?? 'warn' ), 'label' => __( '1. Xác minh Nginx', 'baocache' ), 'detail' => __( 'Kiểm tra runtime/header trước khi bật tối ưu frontend.', 'baocache' ), 'tab' => 'cache', 'action' => __( 'Mở Cache', 'baocache' ) ),
			array( 'state' => (string) ( $status[1]['state'] ?? 'warn' ), 'label' => __( '2. Xác minh Redis', 'baocache' ), 'detail' => __( 'Object cache cần kết nối thật trước khi vận hành production.', 'baocache' ), 'tab' => 'cache', 'action' => __( 'Mở Cache', 'baocache' ) ),
			array( 'state' => $inventory_ready ? 'good' : 'warn', 'label' => __( '3. Quét Asset Inventory', 'baocache' ), 'detail' => $inventory_ready ? __( 'Evidence asset đã sẵn sàng để xem xét.', 'baocache' ) : __( 'Quét một request frontend không đăng nhập trước khi tạo recommendation.', 'baocache' ), 'tab' => 'assets', 'action' => __( 'Mở Assets', 'baocache' ) ),
			array( 'state' => 'neutral', 'label' => __( '4. Cloudflare (tùy chọn)', 'baocache' ), 'detail' => __( 'Chỉ cấu hình secret Coolify khi site thực sự dùng Cloudflare integration.', 'baocache' ), 'tab' => 'cloudflare', 'action' => __( 'Mở Cloudflare', 'baocache' ) ),
		);
		?>
		<section class="baocache-panel baocache-site-diagnostics" data-baocache-pane="dashboard"><div class="baocache-panel__heading"><div><h2><?php esc_html_e( 'Bắt đầu an toàn', 'baocache' ); ?></h2><p><?php esc_html_e( 'Thiết lập theo evidence; không cần bật mọi tối ưu để dùng BaoCache.', 'baocache' ); ?></p></div></div><div class="baocache-site-diagnostics__grid"><?php foreach ( $steps as $step ) : ?><div><span class="baocache-badge is-<?php echo esc_attr( $step['state'] ); ?>"><?php echo esc_html( 'good' === $step['state'] ? __( 'Ready', 'baocache' ) : ( 'neutral' === $step['state'] ? __( 'Optional', 'baocache' ) : __( 'Next', 'baocache' ) ) ); ?></span><strong><?php echo esc_html( $step['label'] ); ?></strong><small><?php echo esc_html( $step['detail'] ); ?></small><button type="button" class="button-link" data-baocache-go="<?php echo esc_attr( $step['tab'] ); ?>"><?php echo esc_html( $step['action'] ); ?></button></div><?php endforeach; ?></div></section>
		<?php
	}
